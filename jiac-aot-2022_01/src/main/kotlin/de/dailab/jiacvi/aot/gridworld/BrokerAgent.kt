...
/**
 * stub for your broker agent
 */
class BrokerAgent(private val brokerId: String): Agent(overrideName=brokerId) {
    ...
    override fun behaviour() = act {
        ...
        */
        on<OrderMessage> { it ->
            val order = it.order
            var workerFree = ""
            var distMax = Int.MAX_VALUE

            // find the closest free worker
            for (worker in workers) {
                if (worker.assignedOrders.isNotEmpty()) continue
                val distance = kotlin.math.abs(worker.position.x - order.position.x) + kotlin.math.abs(worker.position.y - order.position.y)
                if (distance < distMax){
                    distMax = distance
                    workerFree = worker.id
                }
            }
            if (workerFree != "") {
                val serverRef = system.resolve(serverName)
                val workerRef = system.resolve(workerFree)
